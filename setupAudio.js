...
let mic,
  fftRaw,
  fft = [],
  // FFT AND AMP are exported and are eased / avgd with inputs
  FFT = [],
  AMP = [],
  fftAvg,
  fftEase = [],
...
(waveform = []),
  ...
  (onHalfBeat = false),
  (onBar = false),
  (beat_counter_helper = 0));
let BeatDetector;
let ampAvg;
const AMPMULTIDEFAULT = 0.7;
const AMPMULTIMAX = 1;
let minAmp = Number.MAX_VALUE;
let maxAmp = Number.MIN_VALUE;
let melodyAmp = 0;
let melodyEase = 0;
let melodySum = 0;
let melodyAvg = 0;
...
const getAmpMulti = () => {
  try {
    return getMidi(9, AMPMULTIMAX, AMPMULTIDEFAULT);
  } catch (error) {
    return AMPMULTIDEFAULT;
  }
};
...
p5.prototype.setupAudio = function () {
  ...
  fftRaw.setInput(mic);
  BeatDetector = new BeatDetektor(85, 169);
};

AMP_MULTIPLIER = 500;

p5.prototype.updateAudio = function (ampMulti) {
  fftRaw.analyze();
  ampMulti = getAmpMulti();

  amp = ((mic.getLevel() * AMP_MULTIPLIER) / 2) * ampMulti;
  ampStereo.r = mic.amplitude.getLevel(0) * AMP_MULTIPLIER; // average left amplitude
  ampStereo.l = mic.amplitude.getLevel(1) * AMP_MULTIPLIER; // average right amplitude
  melodyAmp = ampStereo.r * ampMulti;
  drumAmp = ampStereo.l * ampMulti;
  melodyEase = ease(melodyAmp, melodyEase || 0, 0.7); // smooth 'amp'
  melodySum += melodyAmp;
  drumEase = ease(drumAmp, drumEase || 0, 0.7); // smooth 'amp'
  ampEase = ease(amp, ampEase || 0, 0.7); // smooth 'amp'
  waveform = fftRaw.waveform(); // array (-1, 1)
  fft = fftRaw.logAverages(fftRaw.getOctaveBands(bands)); // array (0, 255)
  fft = fft.map((f) => f * ampMulti);
  fftEase = fft.map((f, i) => ease(f, fftEase[i] || 0.0, 0.7));

  if (!ampAvg) ampAvg = amp;
  ampAvg = ampAvg + (amp * amp) / 10;
  ampAvg *= C.ampFriction;

  if (!drumAvg) drumAvg = drumAmp;
  drumAvg = drumAvg + (drumAmp * drumAmp) / 10;
  drumAvg *= C.drumFriction;

  if (!melodyAvg) melodyAvg = melodyAmp;
  melodyAvg = melodyAvg + (melodyAmp * melodyAmp) / 10;
  melodyAvg *= C.melodyFriction;

  if (!fftAvg) {
    fftAvg = fft;
  }
  ampCycle += (ampAvg || 0) / 100;
  ampCycle %= 100;

  fftAvg = fftAvg.map((f, i) => {
    if (!f) return fft[i];
    return (f + (fft[i] * fft[i]) / 10) * C.ampFriction;
  });

  const MAGIC_BEAT_NUMBER = 40;

  // only works if divided by CYBER MAGIC
  BeatDetector.process(frameCount / MAGIC_BEAT_NUMBER, fft);
  if (MIDI_BEAT_MODE) {
    beatCount = CBEAT;
    barCount = CBAR;
    onBeat = onCBeat;
    onBar = onCBar;
    onHalfBeat = onCHalfBeat;
  } else {
    onBeat = BeatDetector.beat_counter !== beatCount;
    beatCount = BeatDetector.beat_counter;
    barCount = round(beatCount / 4);
    onBar = beatCount % 4 === 0;
    onHalfBeat = halfBeatCount !== BeatDetector.half_counter;
    halfBeatCount = BeatDetector.half_counter;
  }
...
BeatDetektor.prototype.process = function (timer_seconds, fft_data) {
  ...
  if (this.bpm_timer > this.winning_bpm / 4.0 && this.current_bpm) {
    ...
    if (BD_LOGGING && this.beat_counter % 4 == 0)
      console.log(
        "BeatDetektor(" +
          this.BPM_MIN +
          "," +
